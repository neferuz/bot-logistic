...
class AdminBot:
    ...
    def register_callbacks(self):
        ...
        async def process_folder_selection(callback_query: types.CallbackQuery, state: FSMContext):
            ...
            # Пытаемся получить ссылку из базы
            slug = await self.db.get_folder_link(folder_id)
            
            if not slug:
                # Fallback на жестко заданные ссылки, если в базе пусто
                links_map = {
                    '1': '8udTD2dLVkg0MjYy',
                    '2': 'cI22UnY_7a84ODI6',
                    '3': '_i7riJupfo8wNmUy',
            ...
            try:
                ...
                for chat in check.chats:
                    ...
                    await self.db.add_group(str(chat.id), username=username, title=title)
    ...
    async def send_cargo_results(self, message: Union[types.Message, types.CallbackQuery], page: int, cargo_from: str, cargo_to: str):
        limit = 10
        offset = page * limit
        u_id = message.from_user.id
        ...
        resp += f"🗺 <i>{html.escape(str(cargo_from))} ➔ {html.escape(str(cargo_to))}</i>\n"
        resp += f"⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n\n"
        ...
        for i, (route, s_id, c_link, txt, ts, m_id, r_id) in enumerate(cargo_list):
            ...
            clean_text = str(txt).replace("\n", " ").strip()
            if len(clean_text) > 80: clean_text = clean_text[:77] + "..."
            
            safe_route = html.escape(str(route))
            safe_text = html.escape(clean_text)
            
            resp += f"{num} <b>{safe_route}</b>\n"
            resp += f"└ 📄 <i>{safe_text}</i>\n\n"
        ...
        try:
            if isinstance(message, types.CallbackQuery):
                await message.message.edit_text(resp, reply_markup=builder.as_markup(), parse_mode="HTML")
            else:
                await message.answer(resp, reply_markup=builder.as_markup(), parse_mode="HTML")
        except Exception as e:
            print(f"❌ Ошибка отправки результатов: {e}")
            error_msg = "⚠️ Ошибка отображения результатов. Попробуйте уточнить поиск."
            if isinstance(message, types.CallbackQuery): await message.message.answer(error_msg)
            else: await message.answer(error_msg)
