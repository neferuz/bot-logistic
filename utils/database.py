...
class Database:
    ...
    async def init_db(self):
        async with self._connect() as db:
            ...
            tables_cols = {
                ...
                "cargo_cache": ["route", "text", "chat_link", "msg_id", "group_id", "direction", "message_text", "message_id"],
                "users": ["role"],
    ...
    async def get_paginated_cargo(self, u_id: int, limit: int = 10, offset: int = 0, cargo_from: str = None, cargo_to: str = None, allowed_folders: list = None):
        async with self._connect() as db:
            ...
            # Карта вариаций названий городов
            CITY_VARIATIONS = {
                "ташкент": ["тошкент", "tashkent", "toshkent"],
                "тошкент": ["ташкент", "tashkent", "toshkent"],
                "самарканд": ["samarkand", "samarqand"],
                "бухара": ["buxoro", "bukhoro", "bukhara"],
                "андижан": ["andijan", "andijon"],
                "наманган": ["namangan"],
                "фергана": ["fargona", "fergana"],
                "коканд": ["qoqon", "kokand"],
                "нукус": ["nukus"],
                "ургенч": ["urgench"],
                "хива": ["xiva", "khiva"],
                "навои": ["navoiy", "navoi"],
                "джизак": ["jizzax", "jizzakh", "джизак"]
            }

            def get_search_condition(city_name):
                if is_wildcard(city_name):
                    return None, []
                
                name_lower = city_name.strip().lower()
                variations = [name_lower]
                if name_lower in CITY_VARIATIONS:
                    variations.extend(CITY_VARIATIONS[name_lower])
                
                # Строим OR условие для всех вариаций
                sub_conditions = []
                sub_params = []
                for v in variations:
                    sub_conditions.append(search_pattern)
                    p = f"%{v}%"
                    sub_params.extend([p, p, p, p, v, v])
                
                return f"({' OR '.join(sub_conditions)})", sub_params

            cond_from, params_from = get_search_condition(cargo_from)
            if cond_from:
                conditions.append(cond_from)
                params.extend(params_from)
            
            cond_to, params_to = get_search_condition(cargo_to)
            if cond_to:
                conditions.append(cond_to)
            ...
            async with db.execute(query, params) as cursor:
                ...
                print(f"🔍 [USER {u_id}] Поиск: '{cargo_from}' -> '{cargo_to}' | Найдено: {len(rows)}")
                return rows
    ...
    async def get_total_cargo_count(self, cargo_from: str = None, cargo_to: str = None, allowed_folders: list = None):
        async with self._connect() as db:
            ...
            def get_search_condition(city_name):
                ...
                name_lower = city_name.strip().lower()
                variations = [name_lower]
                # Используем ту же логику как в paginated_cargo (можно вынести в метод класса)
                CITY_VARIATIONS = {
                    "ташкент": ["тошкент", "tashkent", "toshkent"],
                    "тошкент": ["ташкент", "tashkent", "toshkent"],
                    "самарканд": ["samarkand", "samarqand"],
